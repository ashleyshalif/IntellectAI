"""
train.py
Fits PaDiM statistics for each category using ONLY the 'good' training
images. Each category fits its own PCA (80 components) on its own full
448-channel embeddings, and also saves a raw 448-dim centroid (pre-PCA)
...
from padim_core import (FeatureExtractor, load_image, fit_pca, apply_pca,
                         fit_gaussian, save_stats, compute_raw_centroid, DEVICE)
...
def train_category(category: str, extractor: FeatureExtractor, batch_size: int = 8):
    ...
    embeddings = np.concatenate(all_embeddings, axis=0)  # (N, 448, H, W)
    print(f"[{category}] full embedding tensor shape: {embeddings.shape}")

    raw_centroid = compute_raw_centroid(embeddings)
    print(f"[{category}] raw 448-dim centroid computed")

    print(f"[{category}] fitting PCA (80 components) on this category's own features...")
    pca_mean, pca_components = fit_pca(embeddings)
    reduced = apply_pca(embeddings, pca_mean, pca_components)
    print(f"[{category}] reduced embedding tensor shape: {reduced.shape}")

    print(f"[{category}] fitting Gaussian per spatial location (this is the slow step)...")
    mean, inv_cov, hw = fit_gaussian(reduced)

    os.makedirs(MODELS_ROOT, exist_ok=True)
    out_path = os.path.join(MODELS_ROOT, f"{category}_padim.pkl")
    save_stats(out_path, mean, inv_cov, hw, pca_mean, pca_components, raw_centroid)
