...
<style>
    ...
    .metric-value { font-family: 'Space Mono', monospace; font-size: 1.7rem; font-weight: 700; }
    .metric-label { font-family: 'JetBrains Mono', monospace; color: var(--text-dim); font-size: 0.7rem; text-transform: lowercase; letter-spacing: 0.05em; margin-top: 4px; }
...
st.markdown('<div class="status-tag">INSPECTIQ SYS v2.1</div>', unsafe_allow_html=True)
    ...
    if uploaded_files:
        ...
        for i, r in enumerate(reversed(results)):
            with st.expander(f"{r['name']}  —  {r['status']}", expanded=(i == 0)):
                ...
                img_col1, img_col2, metrics_col = st.columns([2, 2, 1])
                with img_col1:
