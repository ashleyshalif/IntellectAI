...
1. Per-category evaluation: AUROC, Precision, Recall, F1 using the MVTec
   test/ split (good + each defect subfolder = anomalous).
2. Cross-Category Structural Robustness Profiling: apply a model trained on
   category A to test images from categories B, C (no retraining) and report
   the same metrics, producing an N x N matrix — this is the experiment
   described in your abstract.
...
from padim_core import FeatureExtractor, load_image, mahalanobis_score_map, load_stats, DEVICE
...
def collect_test_images(category: str):
    """Returns list of (path, label) where label 0 = good, 1 = anomalous."""
...
def score_image(path, extractor, mean, inv_cov, hw):
    ...
    return float(score_map.max())  # image-level score = worst patch


def evaluate_pair(model_category: str, test_category: str, extractor, threshold_percentile=90):
    stats_path = os.path.join(MODELS_ROOT, f"{model_category}_padim.pkl")
    mean, inv_cov, hw = load_stats(stats_path)
    ...
    for path, label in tqdm(items, desc=f"{model_category}->{test_category}", leave=False):
        scores.append(score_image(path, extractor, mean, inv_cov, hw))
        labels.append(label)
    ...
    if len(set(labels)) < 2:
        auroc = float("nan")  # can't compute AUROC with only one class present
    else:
    ...
    return {"auroc": auroc, "precision": precision, "recall": recall, "f1": f1, "n": len(items)}
...
if __name__ == "__main__":
    ...
    for model_cat in cats:
        ...
        for test_cat in cats:
            r = evaluate_pair(model_cat, test_cat, extractor)
    ...
    for (model_cat, test_cat), r in results.items():
        if r:
            print(f"{model_cat} -> {test_cat}: AUROC={r['auroc']:.3f} "
                  f"P={r['precision']:.3f} R={r['recall']:.3f} F1={r['f1']:.3f} (n={r['n']})")
